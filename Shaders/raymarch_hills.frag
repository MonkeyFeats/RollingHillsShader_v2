...
const float THRESHOLD = .003;
const vec2 MOD2 = vec2(3.07965, 7.4235);
const float PI  = 4.0*atan(1.0);
const vec3 sunLight  = normalize( vec3(  0.35, 0.2,  0.3 ) );
const vec3 sunColour = vec3(1.0, .75, .6);
const float fogAmount = 0.0000012;
...
{
  return mix( rgb, GetSky(dir), clamp(dis*dis* fogAmount , 0.0, 1.0));
}

//--------------------------------------------------------------------------
vec3 DE(vec3 p)
{
  float base = Terrain(p.xz).x - 1.1;
  float height =  (Noise(p.xz*2.0)*0.5 + Noise(p.xz)*0.5) - Noise(p.xz);
  //p.y += height;
...
{
  return max(t * .04, 2.0 /resolutionX * (1.0+t));
}
...
void main( )
{
  ...
  if (xy.y < .1 || xy.y >= .9)
  {
    ...
    gl_FragColor=vec4(vec4(0.1));
    return;
  }

  cameraPos = CameraPath(iTime);
  cameraPos.x -= 3.0;
  camTar   = CameraPath(iTime + .009);
  cameraPos.y += Terrain(CameraPath(iTime + .009).xz).x;
  camTar.y = cameraPos.y;
  
  float roll = .15*sin(iTime+.5);
